...
export class Bennynest21703836421186 implements MigrationInterface {
  name = 'Bennynest21703836421186';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`ALTER TABLE "board" DROP CONSTRAINT "fk_user"`);
    await queryRunner.query(
      `ALTER TABLE "user" ADD "permission" character varying NOT NULL DEFAULT 'user'`,
    );
    await queryRunner.query(`ALTER TABLE "board" DROP COLUMN "priority"`);
    await queryRunner.query(
      `ALTER TABLE "board" ADD "priority" "public"."board_priority_enum" NOT NULL DEFAULT 'low'`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE "board" DROP COLUMN "priority"`);
    await queryRunner.query(
      `ALTER TABLE "board" ADD "priority" character varying NOT NULL DEFAULT 'LOW'`,
    );
    await queryRunner.query(`ALTER TABLE "user" DROP COLUMN "permission"`);
    await queryRunner.query(
      `ALTER TABLE "board" ADD CONSTRAINT "fk_user" FOREIGN KEY ("userId") REFERENCES "user"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
  }
}
